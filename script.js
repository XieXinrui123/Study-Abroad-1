...
const fallbackCases = Array.from({ length: 24 }, (_, i) => {
  const countries = ['英国', '香港', '新加坡', '澳洲', '新西兰', '澳门'];
  const degrees = ['本科', '硕士', '博士'];
  const results = ['QS前50', 'QS前100', '奖学金'];
  const schools = ['UCL', 'HKU', 'NUS', 'UNSW', 'Auckland', 'UM'];
  const c = countries[i % 6];
  const d = degrees[i % 3];
  const r = results[i % 3];
  ...
  return {
    id: i + 1,
    title: `${c}${d}申请案例 #${i + 1}`,
    student_school: ['北京科技大学', '中山大学', '西南财经大学', '华东师范大学'][i % 4],
    student_major: ['计算机', '金融', '传播学', '设计'][i % 4],
    gpa: (82 + (i % 12)).toString(),
    language_score: `IELTS ${(6.0 + (i % 4) * 0.5).toFixed(1)}`,
    country: c,
    degree: d,
    result: r,
    offers: Array.from({ length: (i % 4) + 1 }, (_, j) => ({
      school: schools[(i + j) % 6],
      major: ['CS', 'Finance', 'Media', 'Design'][(i + j) % 4],
      rank: results[(i + j) % 3]
    }))
  };
});
...
function setupCases() {
  ...
  (async () => {
    try {
      ...
      state.baseCases = (parsed.length ? parsed : fallbackCases).map(normalizeCase);
